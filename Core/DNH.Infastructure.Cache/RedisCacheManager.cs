    ...
    public partial class RedisCacheManager : IStaticCacheManager
    {
        ...
        protected virtual async Task<T> GetAsync<T>(string key)
        {
            ...
            if (!serializedItem.HasValue)
                return default(T);
            //deserialize item
            var item = JsonSerializer.Deserialize<T>(serializedItem, new JsonSerializerOptions()
            {
        ...
        protected virtual async Task RemoveAsync(string key)
        {
            ...
            if (key.Equals(DNHCachingDefaults.RedisDataProtectionKey, StringComparison.OrdinalIgnoreCase))
                return;
        ...
        protected virtual async Task RemoveByPatternAsync(string pattern)
        {
            ...
            foreach (var endPoint in _connectionWrapper.GetEndPoints())
            {
                ...
                //we should always persist the data protection key list
                keys = keys.Where(key => !key.ToString().Equals(DNHCachingDefaults.RedisDataProtectionKey, StringComparison.OrdinalIgnoreCase));
        ...
        /// <returns>The cached value associated with the specified key</returns>
        public virtual async Task<T> GetAsync<T>(string key, Func<Task<T>> acquire, int? cacheTime = null)
        {
            //item already is in cache, so return it
            if (this.IsSetAsync(key).Result)
                return await this.GetAsync<T>(key);

            //or create it using passed function
            var result = await acquire();

            //and set in cache (if cache time is defined)
            if ((cacheTime ?? DNHCachingDefaults.CacheTime) > 0)
               await this.SetAsync(key, result, cacheTime ?? DNHCachingDefaults.CacheTime);
        ...
        public virtual T Get<T>(string key, Func<T> acquire, int? cacheTime = null)
        {
            ...
            //and set in cache (if cache time is defined)
            if ((cacheTime ?? DNHCachingDefaults.CacheTime) > 0)
                this.SetAsync(key, result, cacheTime ?? DNHCachingDefaults.CacheTime).Wait();
